...
/* This routine is never actually used anywhere. */
#if 0
...
/* which is in position 8 on the stave (bottom line is E = 0)       */
{
  ...
  switch (clef->type) {
  default:
    event_error("unknown clef type in draw_keysig (shouldn't happen)");
  case treble:
    offset = 0;
    break;
  case soprano:
...
/* compute width and height values for all elements in voice */
{
  ...
  struct tuple* thistuple = NULL;
  enum tail_type chordbeaming = nostem;
  int intuple, tuplecount = 0;
  ...
  while (ft != NULL) {
    ...
    case CLEF:
      ...
      case bass:
        ...
        break;
      default:
        event_error("noclef in sizevoice switch (shouldn't happen)");
      };
      if (theclef->octave > 0) {
...
/* beams for grace notes */
{
  ...
  case endbeam:
    ...
    break;
  default:
    event_error("handlegracebeam for nostem (shouldn't happen)");
    break;
  };
}
...
/* notes beamed together */
{
  switch (n->beaming) {
  case nostem:
    event_error("handlebeam for nostem (shouldn't happen)");
    break;
  case single:
    beamctr = -1;
    break;
...
/* one line of specified voice */
{
  ...
  double yend = 0.0, yinstruct = 0.0, ygchord = 0.0, ywords = 0.0;
